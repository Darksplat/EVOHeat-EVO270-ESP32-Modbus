/*
  ...
  Bare-bones Modbus RTU test with Wi-Fi + Arduino OTA + browser monitor
...
void logLine(const String &s) {
  Serial.println(s);
  webLog += s + "\n";
  if (webLog.length() > 14000) webLog.remove(0, webLog.length() - 14000);
}

String hexBufferToString(const uint8_t* buffer, size_t length) {
  String out;
  char tmp[4];
  for (size_t i=0;i<length;i++) {
...
bool readHoldingRegister(uint8_t slave, uint16_t reg, uint16_t& value) {
  ...
  logLine("TX: " + hexBufferToString(request, sizeof(request)));
  ...
  logLine("RX: " + hexBufferToString(response, responseLength));
...
<title>EVO270 Modbus Monitor</title>
<style>body{font-family:system-ui;background:#111;color:#eee;margin:18px}pre{background:#000;padding:12px;white-space:pre-wrap}</style>
</head><body><h1>EVOHeat EVO270 Modbus Monitor</h1><button onclick="fetch('/clear',{method:'POST'})">Clear</button>
<pre id="log">Loading...</pre><script>
async function r(){document.getElementById('log').textContent=await(await fetch('/log?'+Date.now())).text();setTimeout(r,1000)}r();
</script></body></html>)HTML";
    server.send_P(200, "text/html", page);
  });
  server.on("/log", HTTP_GET, [](){ server.send(200, "text/plain", webLog); });
  server.on("/clear", HTTP_POST, [](){ webLog=""; server.send(200, "text/plain", "OK"); });
  server.begin();
}
