...
The exact connector/pigtail used was sourced from **Tempero Systems**:

https://temperosystems.com.au/products/5pin-male-female-jst-sm-locking-pigtail/

Product:

- **5-pin male/female JST-SM locking pigtail**

This gives you the mating connector and prewired leads without having to crimp a new harness from loose terminals.

### The important part: re-pin the connector

The physical connector fitting the EVO270 does **not** guarantee that the supplied wire order is correct for the heat pump.

Before plugging the Tempero pigtail into the EVO270:

1. Put the replacement JST-SM connector beside the original EVO270 connector in the same orientation.
2. Identify each original conductor by function: +12 V, GND, RS485 A, RS485 B and shield/earth.
3. Check the pigtail's wire/cavity order against the EVO270 mapping.
4. Where a pin is in the wrong cavity, use a fine pick or terminal-release tool to gently lift/depress the plastic retaining tang.
5. Pull the crimped terminal and wire out from the rear of the housing.
6. Reinsert it into the required cavity until the terminal locks back into the housing.
7. Repeat until the replacement pigtail matches the EVO270 connector function-for-function.
8. Check continuity and polarity before applying power.

Do **not** plug the pigtail into the heat pump simply because the connector housing fits.

## Alternative: snip and solder

If you do not want to de-pin/re-pin the JST-SM connector, the practical alternative is to splice the pigtail:

1. isolate power to the EVO270;
2. cut the lead with enough length left to work on;
3. match the conductors function-for-function rather than by the replacement pigtail's colour order;
4. solder the required conductors;
5. heat-shrink every conductor individually;
6. add an outer protective sleeve/heat-shrink for strain relief;
7. verify continuity and polarity before reconnecting the heat pump.
...
3. **Tempero JST-SM locking pigtail** — both male/female halves and the supplied wire order visible.
4. **Terminal release** — fine pick/terminal tool releasing one crimped terminal from the JST-SM housing.
5. **Corrected replacement connector** — rear/wire side clearly showing the final EVO270 arrangement.
...
- Tempero 5-pin JST-SM locking pigtail: https://temperosystems.com.au/products/5pin-male-female-jst-sm-locking-pigtail/
...
The wire colours above are what was observed on the tested EVO270-1 installations. Always verify the actual connector/terminal function on the unit being modified rather than assuming the replacement pigtail uses the same colour order.
